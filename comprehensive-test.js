...
async function comprehensiveTest() {
  console.log('🚀 SEHAWQDB 4.0.0 - ALL FEATURES TEST');
  console.log('=========================================\n');
  
  const db = new SehawqDB({
    enableServer: true,
  serverPort: 3001, // Use a different port
    enableRealtime: true,
    debug: true
  });
  ...
  console.log('✅ delete(): Silindi -', !db.has('test:basic'));
  ...
  db.push('todos', { id: 2, text: 'Proje yap', done: false });
  db.push('todos', { id: 3, text: 'Change the world', done: false });
  console.log('✅ push():', db.get('todos').length, 'todo');
  ...
  console.time('Indexli sorgu');
  const indexedResults = db.where('age', '>', 25);
  console.timeEnd('Indexli sorgu');
  ...
  console.log('🔍 Query:', finalStats.query);
  ...
  console.log('\n⏰ 10 saniye sonra kapanacak...');
  
  setTimeout(async () => {
    await db.stop();
    console.log('🛑 Database durduruldu');
    process.exit(0);
  }, 10000);
}
